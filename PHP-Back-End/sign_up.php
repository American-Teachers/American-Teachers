<?php
	...
	if (!empty($data)) {
		...
		*/
		...
		if (isNull($first_name, $last_name, $password, $confirm_password, $email)) {
		...
		if(count($errors) == 0) {
			...
			*/
			...
			$token = generateToken();				
			$user_id = registerUser($pass_hash, $first_name, $last_name, $phone, $email, $active, $token, $user_type);
			if($user_id > 0) {		
				$url = 'http://'.$_SERVER["SERVER_NAME"].'/projects/php/American-Teachers-API/activate_account.php?user_id='.$user_id.'&token='.$token;					
